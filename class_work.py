...
class RegularVerb():
    ...
    '''
    identify conjugation based on principal parts
    ...
    def pres_conj(self, mood, passive=False):
        ...
        # subjunctive mood
            ...
            if self.get_conj() == '1':
                if not passive:
                    conjugatedVerbs = [self.parts[1][:-3] + 'e' + x for x in pers_endings_act_m]
                else:
                    conjugatedVerbs = [self.parts[1][:-3] + 'e' + x for x in pers_endings_pass]

            # 2nd conjugation
            if self.get_conj() == '2':
                if not passive:
                    conjugatedVerbs = [self.parts[1][:-3] + 'ea' + x for x in pers_endings_act_m]
                else:
                    conjugatedVerbs = [self.parts[1][:-3] + 'ea' + x for x in pers_endings_pass]

            # 3rd conj
            elif self.get_conj() == '3':
                if not passive:
                    conjugatedVerbs = [self.parts[1][:-3] + 'a' + x for x in pers_endings_act_m]
                else:
                    conjugatedVerbs = [self.parts[1][:-3] + 'a' + x for x in pers_endings_pass]

            # 3rd io and 4th
            elif self.get_conj() in ['3io', '4']:
                if not passive:
                    conjugatedVerbs = [self.parts[1][:-3] + 'ia' + x for x in pers_endings_act_m]
                else:
                    conjugatedVerbs = [self.parts[1][:-3] + 'ia' + x for x in pers_endings_pass]

            return conjugatedVerbs

    def impf_conj(self, mood, passive=False):

        conjugatedVerbs = []

        # ending groups
        pers_endings_act = ['m', 's', 't', 'mus', 'tis', 'nt']
        pers_endings_pass = ['r', 'ris', 'tur', 'mur', 'mini', 'ntur']
        ...
        if mood == 'ind':
            ...
            # 1st, 2nd, 3rd conj.
            if self.get_conj() in ['1', '2', '3']:
                if not passive:
                    conjugatedVerbs = [self.parts[1][:-2] + 'ba' + x for x in pers_endings_act]
                else:
                    conjugatedVerbs = [self.parts[1][:-2] + 'ba' + x for x in pers_endings_pass]

            # 3rd io and 4th conj.
            elif self.get_conj() in ['3io', '4']:
                if not passive:
                    conjugatedVerbs = [self.parts[1][:-3] + 'ieba' + x for x in pers_endings_act]
                else:
                    conjugatedVerbs = [self.parts[1][:-3] + 'ieba' + x for x in pers_endings_pass]
        ...
        # subjunctive mood
            ...
            # all conjs
            if not passive:
                conjugatedVerbs = [self.parts[1] + x for x in pers_endings_act]
            else:
                conjugatedVerbs = [self.parts[1] + x for x in pers_endings_pass]
...
for v in verbs:
    v = RegularVerb(v)
    print(v.get_conj())
    print(v.pres_conj('ind'))
    print(v.pres_conj('ind', True))
    print(v.pres_conj('subj'))
    print(v.pres_conj('subj', True))
    print(v.impf_conj('ind'))
    print(v.impf_conj('ind', True))
    print(v.impf_conj('subj'))
    print(v.impf_conj('subj', True))
...
#     print('\n\n' + str(d) + '\n')
#     d = RegularVerb(d)
#     print(d.pres_conj('ind'))
#     print(d.pres_conj('ind', True))
#     print(d.pres_conj('subj'))
#     print(d.pres_conj('subj', True))
